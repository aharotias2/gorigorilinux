...
class MyMarkdown {
    ...
    public static function convMdParts($parts) {
    ...
    public static function getMarkdown($filePath, $allowHtml = true) {
	$fp = fopen($filePath, "r");
	if ($fp == null) {
            return "";
	}
	$result = self::getMarkdownByLambda(function () use (&$fp) {
            $line = fgets($fp);
            return $line;
	}, $allowHtml);
	fclose($fp);
    ...
    public static function getMarkdownFromText($text, $allowHtml = true) {
	$lines = explode("\n", $text);
	$lines[] = "&nbsp;\n";
	return self::getMarkdownByLambda(function() use (&$lines) {
            $line = array_shift($lines);
            return $line;
	}, $allowHtml);
    }

    public static function getMarkdownByLambda($getLineFunc, $allowHtml = true) {
	$result = "";
	$prevType = "";
	$line = "";
	$indent = 0;
	...
	while ($line = $getLineFunc()) {
		...
		case "h3":
                    $result .= "<h3>" . self::convMdParts(substr($line, 2)) . "</h3>\n";
                    break;

		case "h4":
                    $result .= "<h4>" . self::convMdParts(substr($line, 3)) . "</h4>\n";
                    break;

		case "h5":
                    $result .= "<h5>" . self::convMdParts(substr($line, 4)) . "</h5>\n";
                    break;
                    
		case "h6":
                    $result .= "<h6>" . self::convMdParts(substr($line, 5)) . "</h6>\n";
                    break;
                    
		case "h7":
                    $result .= "<p><strong><i>" . self::convMdParts(substr($line, 5)) . "</i></strong></p>\n";
                    break;
		...
		case "blockquote":
			...
			$line = $getLineFunc();
                    ...
                    $inBlock[] = " \n";
                    $result .= self::getMarkdownByLambda(function() use (&$inBlock) {
		...
		case "ol":
			...
			$result .= "<ul>\n";
                    ...
                    $result .= "<li>" . self::convMdParts($line) . "</li>\n";
                    break;
		...
		case "table":
			...
			if (preg_match('/^ *\|-+/', $line2)) {
				...
				$result .= "<th>" . self::convMdParts(trim($td)) . "</th>";
                            }
                            $result .= "</tr>\n";
			} else {
                            $result .= "<tr>";
                            ...
                            $line = substr($line, 1, strlen($line) - 2);
                            foreach (explode("|", $line) as $td) {
				$result .= "<td>" . self::convMdParts(trim($td)) . "</td>";
                            }
                            $result .= "</tr>\n<tr>";
                            $line2 = trim($line2);
		...
		case "sample":
			...
			$result .= "<pre class=\"sample\">\n";
                    ...
                    $result .= self::convMdParts(substr($line, 1)) . "\n";
                    break;
		...
		case "code":
			...
			if ($len >= 3 && substr($line, 0, 3) == '```') {
                            ...
                            $result .= self::convMdParts(htmlspecialchars($line));
			}
                    }
                    $result = rtrim($result);
                    $result .= "</pre>\n";
    ...
    public static function putMarkdown($filePath, $allowHtml = true) {
	echo self::getMarkdown($filePath, $allowHtml);
    }
